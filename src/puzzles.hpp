...
void deactivate_timer(frontend *fe)
{
    dbg_fe("deactivate_timer()\n");
    fe->deactivate_timer();
}

void activate_timer(frontend *fe)
{
    dbg_fe("activate_timer()\n");
    fe->activate_timer();
}
...
void cpp_draw_text(void *handle, int x, int y, int fonttype,
                   ...
                   const char *text)
{
    dbg_fe("draw_text(%d, %d, %d, %d, %d, %x, \"%s\")\n",
                      ...
                      int fillcolour, int outlinecolour)
{
    ...
    for (int i = 0; i < 2*npoints; i+=2) {
        dbg_fe("%d, %d", coords[i], coords[i+1]);
        if (i < 2*(npoints-1))
            dbg_fe(",");
    }
    dbg_fe("], ");
...
{
#ifndef NDEBUG
    dbg_fe("draw_circle(%d, %d, %d, ", cx, cy, radius);
    if (fillcolour == -1)
        dbg_fe("-1, ");
    else
        dbg_fe("%x, ", dbg_color(fillcolour));
...
void cpp_draw_thick_line(void *handle, float thickness,
                         ...
                         int colour)
{
    dbg_fe("draw_thick_line(%f, %f, %f, %f, %f, %x)\n",
